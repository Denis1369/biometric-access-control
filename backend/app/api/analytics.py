"""API аналитики, дашборда и журналов событий.

Этот router собирает данные для экранов, где пользователь не редактирует
справочники, а анализирует уже накопленную информацию: журнал проходов,
журнал отслеживания, сводные показатели дашборда, посещаемость сотрудников,
гостевые визиты, нагрузку по камерам и дисциплину по отделам.

Большая часть функций здесь не создаёт новые записи, а строит агрегаты поверх
AccessLog, TrackingLog, Employee, Guest, Camera и Department. Поэтому важно
читать этот файл как слой отчётов: он превращает сырые события распознавания и
прохода в понятные таблицы и графики для администратора или аналитика.
"""

from calendar import monthrange
from datetime import date, datetime
from typing import List

from fastapi import APIRouter, Depends, HTTPException, status
from pydantic import BaseModel
from sqlmodel import Session, func, select

from app.core.database import get_session
from app.core.deps import require_any_permission, require_permissions
from app.core.permissions import (
    ACCESS_LOGS_READ,
    ACCESS_LOGS_READ_RECENT,
    ANALYTICS_READ,
    DASHBOARD_READ,
    TRACKING_LOGS_READ,
)
from app.models.cameras import Camera
from app.models.departments import Department
from app.models.employees import Employee
from app.models.guests import Guest
from app.models.logs import AccessLog, TrackingLog
from app.services.access_log_service import build_person_name, get_recent_access_logs

router = APIRouter(prefix="/api/analytics", tags=["Аналитика и Журналы"])


class AccessLogResponse(BaseModel):
    """Строка журнала проходов для frontend-а.

    В ответе показываются уже подготовленные имена сотрудника/камеры, чтобы
    клиенту не приходилось самостоятельно соединять несколько справочников.
    """

    id: int
    timestamp: datetime
    status: str
    employee_name: str | None
    camera_name: str | None


class TrackingLogResponse(BaseModel):
    """Строка журнала отслеживания человека на камерах.

    TrackingLog отличается от AccessLog тем, что он хранит факт появления на
    камере, а не решение «пустить/не пустить». Эти события используются для
    вероятного маршрута гостя и общей картины перемещений.
    """

    id: int
    timestamp: datetime
    confidence: float
    employee_name: str | None
    camera_name: str | None


class DashboardStatsResponse(BaseModel):
    """Краткие показатели верхнего блока дашборда."""

    total_employees: int
    active_cameras: int
    accesses_today: int


def _parse_target_date(target_date: str | None) -> date:
    """Преобразовать строковый фильтр даты в объект date.

    Многие графики принимают дату из query-параметра в формате YYYY-MM-DD.
    Единый helper нужен, чтобы все endpoint-ы одинаково трактовали пустую дату
    как сегодняшний день и одинаково сообщали пользователю об ошибке формата.

    Параметры:
        target_date: Строка даты из query-параметра или None.

    Возвращает:
        Дата, по которой нужно строить отчёт.

    Ошибки:
        HTTPException: 400, если строка передана не в формате YYYY-MM-DD.
    """

    if not target_date:
        return date.today()

    try:
        return datetime.strptime(target_date, "%Y-%m-%d").date()
    except ValueError as exc:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Параметр target_date должен быть в формате YYYY-MM-DD",
        ) from exc


@router.get(
    "/access-logs",
    response_model=List[AccessLogResponse],
    dependencies=[Depends(require_any_permission(ACCESS_LOGS_READ, ACCESS_LOGS_READ_RECENT))],
)
def get_access_logs(session: Session = Depends(get_session), skip: int = 0, limit: int = 100):
    """Вернуть последние события журнала проходов.

    Endpoint используется на дашборде и в разделе проходной. Он отдаёт события,
    где система приняла решение о доступе: granted или denied. Формирование
    строки вынесено в access_log_service, потому что те же данные нужны и для
    WebSocket-снимка новых событий.

    Параметры:
        session: Сессия БД.
        skip: Смещение для постраничной загрузки.
        limit: Максимальное количество строк журнала.

    Возвращает:
        Список событий доступа с именами людей и камер.
    """

    return get_recent_access_logs(session, skip=skip, limit=limit)


@router.get(
    "/tracking-logs",
    response_model=List[TrackingLogResponse],
    dependencies=[Depends(require_permissions(TRACKING_LOGS_READ))],
)
def get_tracking_logs(session: Session = Depends(get_session), skip: int = 0, limit: int = 100):
    """Вернуть события отслеживания людей на камерах.

    Эти записи показывают, на каких камерах был замечен сотрудник или гость, с
    какой уверенностью и в какое время. Для маршрута гостя TrackingLog является
    основным источником последовательности камер.

    Параметры:
        session: Сессия БД.
        skip: Смещение списка.
        limit: Количество возвращаемых событий.

    Возвращает:
        Список событий TrackingLog, дополненный именами человека и камеры.
    """

    statement = (
        select(TrackingLog, Employee, Guest, Camera)
        .join(Employee, TrackingLog.employee_id == Employee.id, isouter=True)
        .join(Guest, TrackingLog.guest_id == Guest.id, isouter=True)
        .join(Camera, TrackingLog.camera_id == Camera.id, isouter=True)
        .order_by(TrackingLog.timestamp.desc())
        .offset(skip)
        .limit(limit)
    )

    results = session.exec(statement).all()
    response_data = []
    for tracking_log, employee, guest, camera in results:
        response_data.append(
            TrackingLogResponse(
                id=tracking_log.id,
                timestamp=tracking_log.timestamp,
                confidence=tracking_log.confidence,
                employee_name=build_person_name(employee, guest),
                camera_name=camera.name if camera else "Неизвестно",
            )
        )
    return response_data


@router.get(
    "/stats",
    response_model=DashboardStatsResponse,
    dependencies=[Depends(require_permissions(DASHBOARD_READ))],
)
def get_dashboard_stats(target_date: str | None = None, session: Session = Depends(get_session)):
    """Собрать краткую статистику для главного дашборда.

    Сюда входят общее количество сотрудников, количество активных камер и число
    проходов за выбранный день. Такие показатели нужны как быстрый обзор
    состояния системы перед детальными графиками.

    Параметры:
        target_date: Дата отчёта в формате YYYY-MM-DD. Если не передана,
            используется текущий день.
        session: Сессия БД.

    Возвращает:
        DTO с тремя основными счётчиками дашборда.
    """

    total_employees = session.exec(select(func.count(Employee.id))).one()
    active_cameras = session.exec(
        select(func.count(Camera.id)).where(Camera.is_active.is_(True))
    ).one()
    query_date = _parse_target_date(target_date)

    accesses_today = session.exec(
        select(func.count(AccessLog.id)).where(func.date(AccessLog.timestamp) == query_date)
    ).one()

    return DashboardStatsResponse(
        total_employees=total_employees,
        active_cameras=active_cameras,
        accesses_today=accesses_today,
    )


@router.get(
    "/daily-chart",
    dependencies=[Depends(require_permissions(ANALYTICS_READ))],
)
def get_daily_chart(target_date: str | None = None, session: Session = Depends(get_session)):
    """Построить почасовую активность проходов за день.

    Endpoint возвращает массив из 24 значений, даже если в отдельные часы
    событий не было. Это упрощает frontend-график: ему не нужно самостоятельно
    добавлять пропущенные часы.

    Параметры:
        target_date: Дата отчёта в формате YYYY-MM-DD.
        session: Сессия БД.

    Возвращает:
        Словарь labels/data для Chart.js.
    """

    query_date = _parse_target_date(target_date)

    statement = (
        select(
            func.extract("hour", AccessLog.timestamp).label("hour"),
            func.count(AccessLog.id).label("count"),
        )
        .where(func.date(AccessLog.timestamp) == query_date)
        .group_by(func.extract("hour", AccessLog.timestamp))
        .order_by(func.extract("hour", AccessLog.timestamp))
    )
    results = session.exec(statement).all()

    data_dict = {f"{i:02d}:00": 0 for i in range(24)}
    for row in results:
        hour = int(row.hour)
        data_dict[f"{hour:02d}:00"] = row.count

    return {"labels": list(data_dict.keys()), "data": list(data_dict.values())}


@router.get(
    "/monthly-department-chart",
    dependencies=[Depends(require_permissions(ANALYTICS_READ))],
)
def get_monthly_department_chart(session: Session = Depends(get_session)):
    """Построить распределение проходов по отделам за текущий месяц.

    Отчёт помогает увидеть, какие подразделения чаще всего фиксировались в
    системе доступа. Он строится по AccessLog сотрудников и их отделам.

    Параметры:
        session: Сессия БД.

    Возвращает:
        labels/data по названиям отделов.
    """

    today = date.today()
    statement = (
        select(Department.name, func.count(AccessLog.id).label("count"))
        .join(Employee, AccessLog.employee_id == Employee.id)
        .join(Department, Employee.department_id == Department.id)
        .where(func.extract("month", AccessLog.timestamp) == today.month)
        .where(func.extract("year", AccessLog.timestamp) == today.year)
        .group_by(Department.name)
    )
    results = session.exec(statement).all()
    return {"labels": [r.name for r in results], "data": [r.count for r in results]}


@router.get(
    "/monthly-days-chart",
    dependencies=[Depends(require_permissions(ANALYTICS_READ))],
)
def get_monthly_days_chart(session: Session = Depends(get_session)):
    """Построить дневную активность проходов за текущий месяц.

    Возвращает значения для каждого дня месяца, включая дни без событий. Это
    нужно для ровного графика активности и презентабельного дашборда.

    Параметры:
        session: Сессия БД.

    Возвращает:
        labels/data, где labels — номера дней месяца.
    """

    today = date.today()
    _, num_days = monthrange(today.year, today.month)
    statement = (
        select(
            func.extract("day", AccessLog.timestamp).label("day"),
            func.count(AccessLog.id).label("count"),
        )
        .where(func.extract("month", AccessLog.timestamp) == today.month)
        .where(func.extract("year", AccessLog.timestamp) == today.year)
        .group_by(func.extract("day", AccessLog.timestamp))
        .order_by(func.extract("day", AccessLog.timestamp))
    )
    results = session.exec(statement).all()
    data_dict = {str(i): 0 for i in range(1, num_days + 1)}
    for row in results:
        data_dict[str(int(row.day))] = row.count
    return {"labels": list(data_dict.keys()), "data": list(data_dict.values())}


@router.get(
    "/presence",
    dependencies=[Depends(require_permissions(ANALYTICS_READ))],
)
def get_presence(target_date: str | None = None, session: Session = Depends(get_session)):
    """Рассчитать присутствие сотрудников и гостей за выбранный день.

    Присутствующим сотрудник считается, если за день есть хотя бы один granted
    AccessLog с employee_id. Гость считается аналогично по guest_id. Процент
    строится относительно всех активных сотрудников.

    Параметры:
        target_date: Дата отчёта в формате YYYY-MM-DD.
        session: Сессия БД.

    Возвращает:
        Счётчики присутствующих сотрудников, всех активных сотрудников, процент
        присутствия и количество гостей.
    """

    query_date = _parse_target_date(target_date)
    total_active = session.exec(
        select(func.count(Employee.id)).where(Employee.is_active.is_(True))
    ).one()

    present_employees = session.exec(
        select(func.count(func.distinct(AccessLog.employee_id)))
        .where(func.date(AccessLog.timestamp) == query_date)
        .where(AccessLog.status == "granted")
        .where(AccessLog.employee_id.is_not(None))
    ).one()

    present_guests = session.exec(
        select(func.count(func.distinct(AccessLog.guest_id)))
        .where(func.date(AccessLog.timestamp) == query_date)
        .where(AccessLog.status == "granted")
        .where(AccessLog.guest_id.is_not(None))
    ).one()

    percentage = int((present_employees / total_active * 100)) if total_active > 0 else 0
    return {
        "present": present_employees,
        "total": total_active,
        "percentage": percentage,
        "guest_count": present_guests,
    }


@router.get(
    "/camera-traffic",
    dependencies=[Depends(require_permissions(ANALYTICS_READ))],
)
def get_camera_traffic(session: Session = Depends(get_session)):
    """Показать нагрузку по камерам за текущий месяц.

    Камера считается загруженной, если через неё прошло много событий AccessLog.
    Такой отчёт полезен технику и администратору: он показывает, какие точки
    контроля активнее всего используются.

    Параметры:
        session: Сессия БД.

    Возвращает:
        labels/data с названиями камер и количеством событий.
    """

    today = date.today()
    statement = (
        select(Camera.name, func.count(AccessLog.id))
        .join(AccessLog, AccessLog.camera_id == Camera.id)
        .where(func.extract("month", AccessLog.timestamp) == today.month)
        .where(func.extract("year", AccessLog.timestamp) == today.year)
        .group_by(Camera.name)
    )
    results = session.exec(statement).all()
    return {"labels": [r[0] for r in results], "data": [r[1] for r in results]}


@router.get(
    "/daily-attendance",
    dependencies=[Depends(require_permissions(ANALYTICS_READ))],
)
def get_daily_attendance(target_date: date, session: Session = Depends(get_session)):
    """Сформировать дневную посещаемость сотрудников.

    Для каждого активного сотрудника endpoint ищет события входа и выхода за
    выбранную дату. Направление определяется не по биометрии, а по настройке
    камеры: direction == "in" считается входом, direction == "out" — выходом.
    Затем время прихода сравнивается с графиком отдела, чтобы отметить
    опоздание, а время выхода — чтобы отметить ранний уход.

    Параметры:
        target_date: Дата, за которую строится табель присутствия.
        session: Сессия БД.

    Возвращает:
        Список сотрудников с временем входа/выхода, статусом дня и признаками
        нарушения дисциплины.
    """

    employees = session.exec(
        select(Employee, Department)
        .join(Department, isouter=True)
        .where(Employee.is_active.is_(True))
    ).all()
    
    start_of_day = datetime.combine(target_date, datetime.min.time())
    end_of_day = datetime.combine(target_date, datetime.max.time())
    
    logs = session.exec(
        select(AccessLog, Camera)
        .join(Camera)
        .where(AccessLog.timestamp >= start_of_day)
        .where(AccessLog.timestamp <= end_of_day)
        .where(AccessLog.status == "granted")
        .where(AccessLog.employee_id.is_not(None))
        .order_by(AccessLog.timestamp)
    ).all()

    emp_logs = {}
    for log, cam in logs:
        emp_logs.setdefault(log.employee_id, []).append((log, cam))

    result = []
    for emp, dept in employees:
        e_logs = emp_logs.get(emp.id, [])
        
        in_logs = [l for l, c in e_logs if c.direction == "in"]
        out_logs = [l for l, c in e_logs if c.direction == "out"]
        
        time_in_obj = in_logs[0].timestamp if in_logs else None
        time_out_obj = out_logs[-1].timestamp if out_logs else None

        status = "absent"
        if time_in_obj and not time_out_obj:
            status = "working"
        elif time_in_obj and time_out_obj:
            status = "left"

        is_late = False
        left_early = False
        
        if dept:
            if time_in_obj and time_in_obj.time() > dept.work_start:
                is_late = True
            if time_out_obj and time_out_obj.time() < dept.work_end:
                left_early = True

        result.append({
            "id": emp.id,
            "name": f"{emp.last_name} {emp.first_name}",
            "department_id": emp.department_id,
            "date": str(target_date),
            "time_in": time_in_obj.strftime("%H:%M") if time_in_obj else None,
            "time_out": time_out_obj.strftime("%H:%M") if time_out_obj else None,
            "status": status,
            "is_late": is_late,
            "left_early": left_early
        })
        
    return result


@router.get(
    "/daily-guests",
    dependencies=[Depends(require_permissions(ANALYTICS_READ))],
)
def get_daily_guests(target_date: date, session: Session = Depends(get_session)):
    """Сформировать список гостевых визитов за день.

    Гость может войти и выйти несколько раз за день, поэтому endpoint группирует
    события AccessLog по гостю и собирает пары «вход-выход». Если выход есть без
    входа или вход есть без выхода, строка всё равно возвращается, чтобы оператор
    видел неполный визит и мог проверить журнал.

    Параметры:
        target_date: Дата, за которую нужно показать гостевые посещения.
        session: Сессия БД.

    Возвращает:
        Список визитов гостей с временем входа, выхода, длительностью и
        сотрудником, к которому был оформлен визит.
    """

    start_of_day = datetime.combine(target_date, datetime.min.time())
    end_of_day = datetime.combine(target_date, datetime.max.time())
    
    logs = session.exec(
        select(AccessLog, Camera, Guest, Employee)
        .join(Camera, AccessLog.camera_id == Camera.id)
        .join(Guest, AccessLog.guest_id == Guest.id)
        .join(Employee, Guest.employee_id == Employee.id)
        .where(AccessLog.timestamp >= start_of_day)
        .where(AccessLog.timestamp <= end_of_day)
        .where(AccessLog.status == "granted")
        .where(AccessLog.guest_id.is_not(None))
        .order_by(AccessLog.timestamp)
    ).all()

    guest_data = {}
    for log, cam, guest, emp in logs:
        if guest.id not in guest_data:
            guest_data[guest.id] = {"guest": guest, "emp": emp, "logs": []}
        guest_data[guest.id]["logs"].append((log, cam))
        
    result = []
    for gid, data in guest_data.items():
        guest = data["guest"]
        emp = data["emp"]
        g_logs = data["logs"]
        
        current_in_time = None
        visit_counter = 1
        
        for log, cam in g_logs:
            if cam.direction == "in":
                if current_in_time is None:
                    current_in_time = log.timestamp
            
            elif cam.direction == "out":
                if current_in_time is not None:
                    diff = log.timestamp - current_in_time
                    hours, rem = divmod(diff.seconds, 3600)
                    minutes = rem // 60
                    duration = f"{hours} ч {minutes} мин" if hours > 0 else f"{minutes} мин"
                    
                    result.append({
                        "id": f"{guest.id}_{visit_counter}",
                        "name": f"{guest.last_name} {guest.first_name}",
                        "visited_employee": f"{emp.last_name} {emp.first_name}",
                        "date": str(target_date),
                        "time_in": current_in_time.strftime("%H:%M"),
                        "time_out": log.timestamp.strftime("%H:%M"),
                        "duration": duration
                    })
                    current_in_time = None
                    visit_counter += 1
                else:
                    result.append({
                        "id": f"{guest.id}_{visit_counter}",
                        "name": f"{guest.last_name} {guest.first_name}",
                        "visited_employee": f"{emp.last_name} {emp.first_name}",
                        "date": str(target_date),
                        "time_in": None,
                        "time_out": log.timestamp.strftime("%H:%M"),
                        "duration": None
                    })
                    visit_counter += 1
        
        if current_in_time is not None:
            result.append({
                "id": f"{guest.id}_{visit_counter}",
                "name": f"{guest.last_name} {guest.first_name}",
                "visited_employee": f"{emp.last_name} {emp.first_name}",
                "date": str(target_date),
                "time_in": current_in_time.strftime("%H:%M"),
                "time_out": None,
                "duration": None
            })
            
    return result


@router.get(
    "/discipline",
    dependencies=[Depends(require_permissions(ANALYTICS_READ))],
)
def get_discipline_stats(session: Session = Depends(get_session)):
    """Собрать статистику дисциплины по отделам за текущий месяц.

    Алгоритм берёт первый вход каждого сотрудника за каждый день месяца и
    сравнивает его со временем начала работы отдела. Если первый вход позже
    work_start, день считается опозданием. Результат группируется по отделам и
    сотрудникам, чтобы аналитик мог увидеть проблемные подразделения и передать
    информацию HR при необходимости.

    Параметры:
        session: Сессия БД.

    Возвращает:
        Список отделов с количеством своевременных приходов, опозданий и
        детализацией по сотрудникам.
    """

    today = date.today()

    employees_data = session.exec(select(Employee, Department).join(Department)).all()

    dept_stats = {}
    for emp, dept in employees_data:
        if dept.id not in dept_stats:
            dept_stats[dept.id] = {
                "department_id": dept.id,
                "department_name": dept.name,
                "on_time": 0,
                "late": 0,
                "employees": {},
            }
        dept_stats[dept.id]["employees"][emp.id] = {
            "employee_id": emp.id,
            "employee_name": f"{emp.last_name} {emp.first_name}",
            "on_time": 0,
            "late": 0,
            "work_start": dept.work_start,
        }

    statement = (
        select(AccessLog, Camera)
        .join(Camera, AccessLog.camera_id == Camera.id)
        .where(
            func.extract("month", AccessLog.timestamp) == today.month,
            func.extract("year", AccessLog.timestamp) == today.year,
            AccessLog.status == "granted",
            Camera.direction.in_(["in", "both"]),
            AccessLog.employee_id.is_not(None),
        )
    )
    logs = session.exec(statement).all()

    first_arrivals = {}
    for log, _cam in logs:
        if log.employee_id is None:
            continue
        log_date = log.timestamp.date()
        key = (log.employee_id, log_date)
        if key not in first_arrivals or log.timestamp < first_arrivals[key]:
            first_arrivals[key] = log.timestamp

    for (emp_id, _log_date), arrival_time in first_arrivals.items():
        for _, d_data in dept_stats.items():
            if emp_id in d_data["employees"]:
                emp_data = d_data["employees"][emp_id]

                if arrival_time.time() > emp_data["work_start"]:
                    emp_data["late"] += 1
                    d_data["late"] += 1
                else:
                    emp_data["on_time"] += 1
                    d_data["on_time"] += 1
                break

    result = []
    for _, d_data in dept_stats.items():
        d_data["employees"] = list(d_data["employees"].values())
        result.append(d_data)

    return result
