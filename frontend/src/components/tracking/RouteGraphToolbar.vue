...
<script setup>
/**
 * Верхняя панель режимов ручного графа маршрутов.
 *
 * `markup` нужен технику для постановки точек и линий, `path` нужен для ручной
 * проверки кратчайшего маршрута между двумя точками. Компонент только
 * отображает кнопки и счётчики, а вся работа с SVG и backend выполняется в
 * Tracking.vue.
 */
defineProps({
  canEditPlan: {
