...
/**
 * Карточка одного гостевого пропуска.
 *
 * Компонент ничего не загружает сам: он получает уже подготовленного гостя,
 * ссылку на фото и признаки доступных действий от страницы `Guests.vue`.
 * Нажатия на кнопки наружу уходят событиями, потому что именно родительская
 * страница знает, можно ли открыть маршрут, дозагрузить фото полного роста или
 * аннулировать пропуск через backend.
 */
defineProps({
  guest: {
    type: Object,
