...
/**
 * Панель фильтрации списка гостей.
 *
 * Компонент сделан управляемым: он не хранит фильтры внутри себя, а отдаёт
 * изменения через `update:*`. Благодаря этому `Guests.vue` может использовать
 * одни и те же значения фильтров для списка карточек, сортировки и будущего
 * сохранения состояния в query-параметрах, если это понадобится.
 */
defineProps({
  searchQuery: {
    type: String,
